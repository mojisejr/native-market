  ...
  return (
    <Card>
      ...
      <CardContent className="px-4 pb-4 pt-2 sm:px-6 sm:pb-6 sm:pt-3">
        <form action={formAction} className="grid gap-2.5 sm:grid-cols-2 sm:gap-3">
          <Input name="amount" type="number" min={0.01} step="0.01" placeholder="จำนวนเงิน" required />
          <select
            name="paymentMethod"
            defaultValue="cash"
            className="h-10 rounded-2xl border border-white/10 bg-white/10 px-3 py-2 text-sm text-white outline-none focus-visible:ring-2 focus-visible:ring-accent"
          >
